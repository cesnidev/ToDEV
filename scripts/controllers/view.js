...
calcomm.controller('ViewCtrl',function($scope,Session,CalcommLogin,$rootScope,$location,CalcommConfig,$http,CalcommUtils,CalcommResource){
...
//		$http.put('http://45.55.30.212/api/v1/users/2/approved',{token:Session.getSession().token,app_id:CalcommConfig.AppId},{"headers" : "Content-Type=application/x-www-form-urlencoded; charset=UTF-8"})
				...
				$http({
					...
					if($scope.groups.length>0){//existen grupos en la BD
						angular.forEach($scope.groups,function(group){
							group.upload=false;
							group.status=0;//0 sin modificar; 1 editado/existente; 2  nuevo/modificado;3 eliminado/existente;  4 nuevo/eliminado
						});
					}
