calcomm.factory('CalcommResource', function($resource,CalcommConfig,Session) {
	return $resource("http://"+CalcommConfig.IP+"/api/v1/", {
		...
		approve:{
			method:'PUT',
			params:{token:Session.getSession().token,app_id:CalcommConfig.AppId},
			url:'http://'+CalcommConfig.IP+'/api/v1/users/'+Session.getSession().id+'/approved'
		},
		archive:{
			method:'PUT',
			params:{token:Session.getSession().token,app_id:CalcommConfig.AppId},
			url:'http://'+CalcommConfig.IP+'/api/v1/users/'+Session.getSession().id+'/archived'
		}
	});
})
.factory('CalcommUtils', function(Session,$location,$http,CalcommConfig) {
	var calcommutils = {};
	
	calcommutils.logout = function() {
	...
	calcommutils.getProfileImage = function(url){
		if(url){
	...
	calcommutils.getMyStatus = function(){
		var session = Session.getSession();
		if(session){
			if(session.status == 'incomplete'|| session.status == 'archived')
				return 'Candidate';
			else
				return 'Staff';
		}
...
//				console.debug(cookie);
	...
	calcommlogin.getId = function () {
		if(calcommlogin.isAuthenticated())
		return Session.getSession().id;//validar campos validos
		else
		return -1;
	};
